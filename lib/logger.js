...
function getLogger(categoryName) {
	...
	var pLogger = {};
	// FIX only get symbol propertys, can't get Class's self propertys.
	// for (var key in logger) {
	// 	pLogger[key] = logger[key];
	// }

	//self or symbol propertys
	Reflect.ownKeys(Object.getPrototypeOf(logger)).forEach(function (item) {
		//ignore private or constructor func
		if (typeof (logger[item]) == "function" && item != 'constructor' && !item.startsWith("_")) {
			// console.debug("========logger.%s()", item)
...
function configure(config, opts) {
	...
	// FIX configure use new conf object
...
module.exports = {
	formatAsDate: function (format, date) {
	...
	isConfigured: log4js.isConfigured,
	configure: configure,
